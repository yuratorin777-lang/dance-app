...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Блок-напоминание о подписи чека
    const receiptNotice = `\n\n📌 <b>ВАЖНО ПРИ ОПЛАТЕ:</b>\n` +
                          `Если вы отправляете чек напрямую администратору или прикрепляете в ЛК, обязательно подписывайте к чеку <b>ФИО ребёнка</b> (<i>${studentName}</i>), чтобы мы сразу зачислили занятия!`;
    ...
    if (triggerReason === 'balance_one') {
      ...
      // 3. Ушел в минус (Прозрачно и лояльно)
      const absBalance = Math.abs(balanceLessons);
      text = `${mentionHeader}🤝 <b>Внимание: отрицательный баланс</b>\n\n` +
             `Ученик: <b>${studentName}</b>\n` +
             `Текущий остаток: <b>-${absBalance} зан.</b> (посетили в долг / в счёт нового абонемента)\n\n` +
             `Пожалуйста, оформите новый абонемент в ближайшее время. Спасибо, что вы с нами! ❤️` +
             receiptNotice;
